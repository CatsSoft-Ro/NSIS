...
; KF_FLAG_ dwFlags values for use with SHGetKnownFolderPath force creation of the specified folder if it does not exist
; Флаги, указывающие на то, какую директорию необходимо вернуть функции в случае, если директория, указанная в CSIDL 
;не существует или была перемещена или переименована.
...
!macro _SHGetKnownFolderPath rfid dwFlags outVar
	...
	${If} $0 == 0
	;lstrlenW - UNICODE версия, возвращает длину строки в словах (lstrlenA возвращает длину строки в байтах )
		System::Call 'kernel32::lstrlenW(i $1) i.r2'
		IntOp $2 $2 * 2
		System::Call '*$1(&w$2 .s)'
		;CoTaskMemFree освобождает определенные типы задействованной памяти
