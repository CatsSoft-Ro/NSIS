...
TCHAR *pszAppName; // Section Name (Temporary)
...
int WINAPI myatoi(TCHAR *s)
{
  ...
  TCHAR t='9'; // cap top of numbers at 9
...
void WINAPI atoIO(TCHAR *str) {
  ...
  while (*p1)
  {
    ...
    switch (*(LPDWORD)p1)
    {
    case CHAR2_TO_DWORD('\\', 't'):
      *p2 = '\t';
      p1 += 2;
      p2++;
      break;
    case CHAR2_TO_DWORD('\\', 'n'):
      *p2 = '\n';
      p1 += 2;
      p2++;
      break;
    case CHAR2_TO_DWORD('\\', 'r'):
      *p2 = '\r';
      p1 += 2;
      p2++;
      break;
...
TCHAR* WINAPI IOtoa(TCHAR *str)
{
  ...
  TCHAR *newstr = (TCHAR*)MALLOC(strlen(str)*2+1);
  TCHAR *p1, *p2;
