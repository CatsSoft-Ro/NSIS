...
bool silent, popup, resume, nocancel, noproxy, nocookies, convToStack;
		...
		FTP_TRANSFER_TYPE_BINARY|INTERNET_FLAG_RELOAD,0)) == NULL)
	{
		...
		*buf = 0;
		...
		if(fput && (_tcsstr(buf, TEXT("550")) != NULL ||  _tcsstr(buf, TEXT("553")) != NULL))
		{
			...
			movp = path + 1;
			if(*movp == TEXT('/')) movp++; // don't need to creat root
			while(_tcschr(movp, TEXT('/')))
			{
				*_tcschr(movp,TEXT('/')) = 0;
...
DWORD __stdcall inetTransfer(void *hw)
{
	...
	if((hSes = InternetOpen(szUserAgent, openType, szProxy, NULL, 0)) != NULL)
	{
		...
		if(hInstance = LoadLibrary(TEXT("wininet.dll")))
...
void progress_callback(void)
{
    ...
    _tcsrchr(fn, TEXT('\\')) ? _tcsrchr(fn, TEXT('\\')) + 1 : fn);
	else wsprintf(buf, TEXT("%s"),szCaption);
